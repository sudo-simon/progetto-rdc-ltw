...
setInterval(function(){ 
    ...
    var chatList=JSON.parse(localStorage.user).chat;
  ...
  $("#invia").click(function(){
    ...
    var index= JSON.parse(localStorage.user).chat.findIndex(arr => arr.includes(chat_name));
    var ex=JSON.parse(localStorage.user).chat[index][1];
    var toSend={
      messaggio:messaggio,
      ex:ex
    }
    ...
    $.ajax({
      ...
      url: 'http://localhost:8887/chat/invia',						
...
$('.goBackChat').trigger('click'); 
  ...
  $.ajax({
    ...
    data: JSON.stringify(chat),
        contentType: 'application/json',
                url: 'http://localhost:8887/chat/creaChat',						
                success: function(data) {
...
function caricaListaChat(){
  ...
  var l=JSON.parse(localStorage.user).chat;
  if (l!=[]){
  for (var i in l){
    var c=l[i][0];
    ...
    $("#listaAggiungiAmici").append('<li class="list-group-item" style="height: 50px;"> \
                                      <div class="custom-control custom-checkbox" style="height: 31px;text-align: left;">\
                                        <span>\
                                          ...
                                          <label class="cursor-pointer font-italic custom-control-label" for="customCheck'+c+'" style="font-size: 22px;margin-left: 5px;">'+c+'</label>\
                                        </span>\
                                      </div>\
                                    </li>')
    }
...
function updateLocalStorage(toRef,key,after){
  $.ajax({
    ...
    contentType: 'application/json',
    url: 'http://localhost:8887/chat/update',						
    success: function(data) {
...
function settingChatClick(n) {
  $(".settingChat").click(function(){
    ...
    var index= JSON.parse(localStorage.user).chat.findIndex(arr => arr.includes(chat));
    var ex=JSON.parse(localStorage.user).chat[index][1];
    $.ajax({
      type: 'POST',
      data: JSON.stringify({
      ...
      contentType: 'application/json',
      url: 'http://localhost:8887/chat/esci',						
      success: function(data) {
          console.log("uscito dalla chat");
          var x=JSON.parse(localStorage.user);
          var index= x.chat.findIndex(arr => arr.includes(chat));
          x.chat.splice(index,1);
...
function ascoltaChat(item){
  ...
  $.ajax({
    ...
    contentType: 'application/json',
    url: 'http://localhost:8887/chat/ascolta',						
    success: function(data) {
  ...
  var index= JSON.parse(localStorage.user).chat.findIndex(arr => arr.includes(item.chat_id));
  var nc=JSON.parse(localStorage.user).chat[index][0];
  if (nc==document.getElementsByClassName("chatNome")[0].innerText){
      if (JSON.parse(localStorage.getItem("codaChat:"+item.user+item.chat_id)).to_consume=="y"){
        ...
        $.ajax({
          ...
          contentType: 'application/json',
          url: 'http://localhost:8887/chat/consume',						
          success: function(data) {
